...
class SearchEngine:
    ...
    #################FOR iterative_deepening_astar
    def iterative_deepening_astar(self, max_depth, costbound=None, timebound=80):
        #iteratively increase the depth to the maximum depth (depth)
        goal_node = []
        initial_depth = 0
        self.set_strategy('breadth_first', 'full')
        while(initial_depth < max_depth):
            ###NOW do the search and return the result
            self.search_start_time = os.times()[0]
            self.search_stop_time = None
            if max_depth or timebound:
                self.search_stop_time = self.search_start_time + timebound
            goal_node = self._searchOpen(self.goal_fn, self.heur_fn, self.fval_function, costbound)
